...
> **Comparison to the existing archaeal census.** Payá & Bonete (2023) manually surveyed 109 cultured archaeal species and found every genome to encode one to three Lsm proteins (Nanoarchaeota 100% single-copy; Euryarchaeota mostly one; Crenarchaeota often two–three). Our result extends that picture to the reduced-genome, MAG-dominated DPANN and Asgard lineages they did not sample — which likewise retain Sm/Lsm — and contrasts it against the bacterial CPR radiation, which has lost the fold. The two censuses agree where they overlap (e.g., Nanoarchaeota single-copy retention).
