...
iqtree3 -s 3-analysis/smlsm_curated_aln_nr90_trim.fasta \
        ...
        --prefix 4-results/smlsm_tree_curated_nr90   # 実行中（無根 ML）
```

**方針：** 有根化は後段の構造アンカー工程（古細菌 SmAP / 細菌 Hfq 相互外群、design memo §1）で実施。この初回 ML は無根で樹形の健全性（コンタミ除去でクレードが整理されたか）を確認する位置づけ。**次段階の "本番" アライメントは PROMALS3D/Expresso による構造ガイド MSA に置換予定**（trim 版は暫定）。選択モデル `Q.INSECT+G4`（BIC）。
...
- DPANN 9系統＋Asgard 5系統を Entrez assembly（`latest[Filter]`＝GenBank MAG 含む）で取得。
- **アノテーション交絡の除去：** DPANN は大半が MAG で protein.faa.gz を持たない。protein.faa があればそれを使い、**無ければ genomic.fna を Prodigal（meta モード）で ab initio 予測**してタンパクを得る。→「真の欠如」がアノテーション欠如の artifact でないことを担保。
- 出力：`1-downloaded-data/proteomes/<acc>.faa`（ヘッダに `taxon|accession|` タグ）＋ `dpann_asgard_manifest.tsv`（taxon/species/accession/source/n_proteins）。
- 系統規模（latest[Filter]）：Woesearchaeota 1986・Asgardarchaeota 1933・Micrarchaeota 1747・Nanoarchaeota 922… → `--cap N`（既定40）でサブサンプリング。

**パイロット結果（Nanohaloarchaeota, 8 genome, 9760 タンパク）：**
- PF01423 = **6 ヒット**（6/8 genome が単一コピー LSM、Payá&Bonete の Euryarchaeota パターンと一致）、PF17209 = **0**（Hfq 型なし）。
- ヒット1件は **Prodigal 予測ゲノム由来**（GCA_030643105.1）＝アノテーションでは見逃す本物の Sm/Lsm を回収、手法の核心的価値を実証。
- スコア E ~1e-15〜1e-19（GA 大幅超）、注釈 "LSm family protein" で真正。

**→ パイプライン全体（取得＋Prodigal＋二重HMM）が end-to-end で検証済み。** 次段階：全 DPANN/Asgard へスケール（`--cap` 調整）→ 型判定・コピー数集計 → Payá&Bonete 109古細菌マトリックスと比較 → ヒット配列を §2 構造ガイドアライメント経由で §1 有根ツリーへ。
